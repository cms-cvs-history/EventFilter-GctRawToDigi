...
#  filename = cms.untracked.string ( "eventsForEmyr2.txt" ),
  filename = cms.untracked.string("/home/jbrooke/patternCaptureOrbit_ts__2008_08_15__15h05m43s.txt"),
...
process.l1tgct.outputFile = cms.untracked.string('l1tgct.root')

process.load('DQM.L1TMonitor.L1TRCT_cfi')
process.l1trct.disableROOToutput = cms.untracked.bool(False)
process.l1trct.outputFile = cms.untracked.string('l1trct.root')

process.p = cms.Path ( process.gctRaw * process.dumpRaw * process.l1GctHwDigis * process.dumpGctDigis )

process.p = cms.Path ( process.gctRaw * process.dumpRaw * process.l1GctHwDigis * process.l1trct * process.l1tgct )
